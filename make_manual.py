...
def slide_cover(prs):
    ...
    add_text(slide, Inches(0.8), Inches(5.6), Inches(12), Inches(0.4),
             "Version 1.3", size=13, color=PRIMARY_LIGHT)
...
def slide_toc(prs, total):
    ...
    sections = [
        ("01", "시작하기", "프로그램 소개 · 준비사항 · 첫 실행 · 자동 업데이트", PRIMARY),
        ("02", "조회하기", "매입 세금계산서 조회 · 필터링 · PDF 저장 · 엑셀 다운로드", SUCCESS),
        ("03", "매핑 관리", "거래처별 계정/적요 매핑 · 품목 규칙 설정", ACCENT),
        ("04", "GMS 자동화", "전표 입력 · 영세율 처리 · 옵션 · 트러블슈팅", PRIMARY_DARK),
    ]
...
def main():
    ...
    slide_content(
        ...
        "프로그램 첫 실행",
        "exe 실행 → MS 로그인 → 조회 순서로 진행됩니다",
        [
            f"{APP_NAME}.exe 더블클릭으로 실행",
            "Windows Defender 경고 시 '추가 정보' → '실행'",
            ("서명이 없는 exe 라 처음에만 경고가 뜰 수 있음", 1),
            "자동으로 MS 로그인 브라우저가 열립니다",
            ("O365 회사 계정으로 로그인 (이메일 · 비밀번호 · MFA)", 1),
            ("로그인 전까지 조회 버튼이 비활성화됩니다", 1),
            "로그인 성공 → 타이틀바에 사용자명 표시",
            ("이후 조회/GMS 전표 입력 모두 이 세션으로 동작", 1),
            "조회 버튼이 활성화되면 정상 사용 가능",
        ],
        tip="한 번 로그인하면 세션이 저장되어 다음 실행부터는 자동 로그인됩니다",
    )
    ...
    slide_content(
        ...
        [
            ...
            "⑥ 액션 버튼",
            ("전체선택 · 거래처 매핑 · 엑셀 다운로드 · 저장(PDF) · GMS 전표 입력", 1),
        ],
    )

    # 8. 자동 업데이트
    slide_content(
        prs, "Section 01 · 시작하기", 8, total,
        "자동 업데이트 확인",
        "새 버전이 나오면 자동으로 알려줍니다",
        [
            "프로그램 실행 시 자동으로 최신 버전 확인",
            ("인터넷 연결 시 백그라운드에서 확인 — 작업에 방해 없음", 1),
            "새 버전이 있으면 알림 팝업 표시",
            ("'현재 v1.2.0 → 최신 v1.3.0' 형태로 안내", 1),
            "'예' 클릭 시 다운로드 페이지가 브라우저에서 열림",
            "새 exe 를 다운로드 → 기존 파일 교체 → 끝",
            "네트워크 오류 시 알림 없이 정상 진행",
            ("오프라인 환경에서도 문제없이 사용 가능", 1),
        ],
        tip="타이틀바와 하단 상태바에서 현재 버전을 확인할 수 있습니다",
    )
    ...
    slide_section(
        prs, "02", "SECTION 02",
        "조회하기",
        "매입 세금계산서 조회 · 필터링 · PDF 저장 · 엑셀 다운로드",
        9, total, SUCCESS,
    )
    ...
    slide_content(
        ...
        [
            ...
            "작성일자 · 발행일자",
            ("세금계산서 작성/발행 날짜 · GMS 증빙일자는 작성일자 기준", 1),
            "공급자 · 사업자번호",
            ("거래처 정보", 1),
            "유형 · 과세",
            ("일반/수정 · 과세/면세/영세", 1),
    ...
    # 14. 엑셀 다운로드
    slide_content(
        prs, "Section 02 · 조회하기", 14, total,
        "엑셀 다운로드",
        "조회된 내역을 엑셀 파일로 저장합니다",
        [
            "[엑셀 다운로드] 초록색 버튼 클릭",
            "저장 위치 · 파일명 선택",
            ("기본 파일명: 세금계산서_시작일_종료일.xlsx", 1),
            "현재 필터가 적용된 결과가 저장됨",
            ("필터를 사용 중이면 필터된 데이터만 저장", 1),
            "포함 항목",
            ("작성일자 · 발행일자 · 공급자 · 사업자번호 · 유형 · 과세구분", 1),
            ("품목 · 비고 · 공급가액 · 세액 · 합계금액 · 승인번호", 1),
            "금액은 숫자로 저장 (엑셀에서 합계/정렬 가능)",
            "자동 필터 포함 — 엑셀에서 바로 필터링 가능",
        ],
        tip="파일이 이미 열려있으면 저장이 실패합니다. 기존 파일을 닫고 다시 시도하세요",
    )
    ...
    slide_content(
        ...
        "로그인과 GMS 세션",
        "앱 시작 시 로그인이 GMS 전표 입력에도 그대로 사용됩니다",
        [
            "앱 시작 시 MS 로그인 = GMS 세션 동시 확보",
            ("별도의 GMS 로그인 불필요", 1),
            "세션이 유효하면 전표 입력 시 바로 진행",
            ("백그라운드 모드에서도 로그인 없이 동작", 1),
            "세션 만료 시 (약 수 시간 ~ 1일)",
            ("앱 재시작 또는 로그인 버튼 클릭으로 갱신", 1),
            ("GMS 전표 입력 중 만료되면 브라우저가 열려 재로그인 요청", 1),
            "세션 파일: %APPDATA%\\PopbillTaxInvoice\\gms_state.json",
            ("PC별로 저장 · 다른 PC에서는 재로그인 필요", 1),
        ],
        tip="세션이 만료되어도 데이터 손실은 없습니다. 재로그인 후 이어서 진행됩니다",
    )
    ...
    # 22. 영세율 처리
    slide_content(
        prs, "Section 04 · GMS 자동화", 22, total,
        "영세율 세금계산서 처리",
        "영세율 건은 자동으로 구분하여 처리합니다",
        [
            "과세구분이 '영세' 인 건 자동 감지",
            "GMS TX/증빙구분을 '영세율' 로 자동 선택",
            ("과세 건은 기존과 동일하게 '세금계산서' 선택", 1),
            "부가세 입력 생략 (영세율은 부가세 0원)",
            "공급가액만 입력됨",
            "별도 설정 불필요 — 자동 처리",
        ],
        tip="영세율 · 과세 · 면세가 섞여 있어도 건별로 자동 구분합니다",
    )

    # 23. 옵션 설명
    slide_content(
        prs, "Section 04 · GMS 자동화", 23, total,
        "GMS 옵션",
        "상황에 맞게 옵션을 조합하세요",
        [
            "백그라운드 실행",
    ...
    # 23. FAQ
    slide_content(
        prs, "Section 04 · GMS 자동화", 24, total,
        "자주 묻는 질문",
        "문제가 생기면 여기부터 확인해보세요",
        [
